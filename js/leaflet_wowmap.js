...
(function()
{
	...
	var Elements =
	{
		...
		TechDetails: document.getElementById('js-techdetails'),
	...
	offsxhr.open( 'GET', '/offsets.json', true );
	...
	function InitializeMapOptions( maps )
	{
		...
		maps.forEach( function( map, i )
		{
			...
			if( i === 0 || map.internal === decodeURIComponent(url[ 1 ]) )
			{
				...
				if( map.internal === decodeURIComponent(url[ 1 ]) )
				{
		...
		// Get map coordinates
		if(parseFloat( url[ 4 ] ) && parseFloat( url[ 5 ] ) ){
			var latlng = new L.LatLng( parseFloat( url[ 4 ] ), parseFloat( url[ 5 ] ) );
		}
	...
	function InitializeEvents()
	{
		...
		Elements.TechBox.addEventListener( 'click', function( )
		{
		...
		{
			if(!Elements.TechBox.checked) return;
	...
	function SynchronizeTitleAndURL( isMapChange )
	{
		...
		var url = '/' + Current.InternalMap + '/' + Current.Version + '/' + zoom + '/' + latlng.lat.toFixed(3) + '/' + latlng.lng.toFixed(3);
